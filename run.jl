...
function parse_args()
    args = ARGS
    if length(args) < 2
        println("用法: julia run.jl <L> <sigma> [选项]")
        println("  <L>      系统尺寸（整数）")
        ...
        println("  --datadir <dir> 数据目录（默认 data）")
        exit(1)
    end
...
hist_counts    = zeros(Float64, NBINS)          # |h_i - h_j| histogram（累积）
hk_power_sum   = zeros(Float64, L, L)           # Σ |h_k|² over sweeps
...
for i in 1:MEASURE
    ...
    if i % 10000 == 0
        println("  sweep $i / $MEASURE  W²=$(round(W2_series[i], digits=4))")
    end
end
...
# χ' = <|h_k|²> - |<h_k>|²（connected susceptibility）
...
println("\n========== 结果 ==========")
println("  W²   = $(round(W2_mean,  digits=5)) ± $(round(W2_err,  digits=5))  ρ(1)=$(round(W2_rho1,  digits=4))  block=$(W2_bs)")
...
println("  histogram: $(sum(hist_counts)) 键/sweep（理论 $(2*L*L)）")

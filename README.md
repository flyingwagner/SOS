...
| `SOSCore.jl`        | MC 主循环、热化、测量间隔、自相关控制。 |
| `Measure.jl`        | 单次构型上的观测量计算：$W^2$、$G(\mathbf{r})$、$h_{\mathbf{k}}$、能量密度等。 |
...
| File | 作用 |
|---|---|
| `run.jl`                 | 主模拟入口：扫描 $(L,\sigma)$，输出到 `data/` 下 `L{L}_s{σ}.jld2`。 |
...
| File | 作用 |
|---|---|
| `fit_analysis.jl`         | $W^2(L)$、$\chi_{\mathbf{k}}(L)$ 等关于 $L$ 的标度拟合（对数-对数回归）。 |
| `plot_results.jl`         | 主结果图：$W^2$-$L$、$\chi$、$\chi'$ 的有限尺寸标度。 |
...
| 目录 | 内容 | 生成命令 |
|---|---|---|
| `data/`       | 主模拟：$W^2$、$G$、$\chi_{\mathbf{k}}$ for $(L,\sigma)$ | `julia run.jl` |
...
`further_simulations.md` 列出尚需重跑以补齐 `hk_power_err` 字段的 $(L,\sigma)$ 组合。
...
1. 主模拟循环和核心观测量（$W^2$、$G(\mathbf{r})$、$\chi_{\mathbf{k}}$、能量）实现并跑通 $(L,\sigma) \in \{4,\dots,512\} \times \{0.5,1.0,2.0,4.0\}$。
2. 有限尺寸标度拟合：$W^2(L) \sim \log L$ 的粗糙相行为在 $\sigma = 2$ 处验证，其它 $\sigma$ 下提取有效指数。
...
- 重新运行 `run_measure_extra.jl` 覆盖全部 12 组 $(L,\sigma)$ 以使 `hk_power_err` 字段写入所有数据文件（见 `further_simulations.md`）；目前只有部分文件有误差数据，因此 Fig 4 的误差棒在部分 $\sigma$ 上缺失。
